...
export const loginUser = TryCatch(async (req, resizeBy, next) => {
...
export const forgotPassword = TryCatch(async(req, res, next)=>{
    ...
    if(users.length === 0){
        return res.json({
            message: "If that email exists, we will sent a reset link",
        });
    }
    ...
    const message = {
        to: email,
        subject: "Reset Your Password - Tech-job",
...
export const resetPassword = TryCatch(async (req, res, next) => {
    ...
    const storedToken = await redisClint.get(`forgot:${email}`);
    ...
    await redisClint.del(`forgot:${email}`);
